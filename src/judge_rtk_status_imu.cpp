...
public:
    GPSSubscriber() : Node("judge_rtk_status_imu") {
        ...
        imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
            "/imu/data_raw", 10, std::bind(&GPSSubscriber::imu_callback, this, _1));
...
private:
    void timer_callback()
    {
        ...
        gnss_flag = true;
        status_flag = false;
        if(emcl_flag && ublox_flag && gnss_flag && odrive_flag && imu_flag){
            ...
            if(status_flag){
                ...
                if(vx > 0.2 && dist_gnss > 0.001 && dist_gnss < 3.0){
                    ...
                    if(std::abs(pre_yaw - temp_gnss_yaw) > M_PI/5){
                        yaw += diff_imu_yaw;
                        RCLCPP_INFO(this->get_logger(), "imu_yaw: %lf", yaw);
                    }else{
                        yaw = temp_gnss_yaw;
    ...
    void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        diff_imu_yaw = imu_yaw - pre_imu_yaw;
    ...
    */
    ...
    double x, y, gnss_x, gnss_y, odrive_x, odrive_y, odrive_yaw, yaw, vx, vth, temp_gnss_yaw, switch_yaw, imu_yaw;
    ...
    double pre_odrive_x = 0.0, pre_odrive_y = 0.0, pre_odrive_yaw = 0.0, diff_odrive_x = 0.0, diff_odrive_y = 0.0, diff_odrive_yaw = 0.0, pre_gnss_x = 0.0, pre_gnss_y = 0.0, pre_yaw = 0.0, pre_imu_yaw = 0.0, diff_imu_yaw = 0.0;

    bool status_flag = false, ublox_flag = false, gnss_flag = false, odrive_flag = false, emcl_flag = false, switch_flag = true, imu_flag = false;
    int gnss_count = 0;
};
